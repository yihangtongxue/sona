"""Run manually: encrypted Ed25519 key creation and release sidecar signing."""
...
def sign(args):
    if args.confirm_version != VERSION:
        raise ValueError(f"请明确确认当前版本 {VERSION}。")
    ...
    if base64.b64decode(record["publicKey"]) != public:
        raise ValueError("私钥与打包公钥不匹配。")
    prepared = []
    for artifact in artifacts:
        if artifact.suffix not in {".zip", ".dmg"} or not artifact.is_file():
            raise ValueError("只允许签名 ZIP 或 DMG 安装包。")
        output = artifact.with_name(artifact.name + ".sig.json")
        if output.exists():
            raise ValueError(f"签名文件已存在：{output.name}。不要覆盖已发布产物。")
        if artifact.suffix == ".zip":
            with zipfile.ZipFile(artifact) as archive:
                info = plistlib.loads(archive.read("Sona.app/Contents/Info.plist"))
                bundled_key = json.loads(archive.read("Sona.app/Contents/Resources/sona/updates/update-public-key.json"))
            if (info.get("CFBundleIdentifier") != BUNDLE_ID or info.get("CFBundleVersion") != VERSION
                    or info.get("CFBundleShortVersionString") != VERSION or bundled_key != record):
                raise ValueError("ZIP 内的应用身份、版本或公钥不匹配。")
        ...
        if not 0 < size <= 4 * 1024**3:
            raise ValueError("安装包大小不在支持范围内。")
        asset = {"platform": "macos", "architecture": "arm64", "packageType": artifact.suffix[1:],
                 "fileName": artifact.name, "size": size, "sha256": digest.hexdigest()}
    ...
    print("在 ReleaseHub 中选择原安装包，保留旁边的 .sig.json；不要修改已签名安装包。")
