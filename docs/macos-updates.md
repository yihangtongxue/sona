...
当前为待验收的代码和配置，不代表已经生成安装包或完成升级验证。未经用户确认，不运行密钥生成、打包、提交、推送或发布。
...
- 当前构建：Apple 芯片、macOS 26+。本机已锁定 mlx-metal 0.32.2 wheel 标记为 macOS 26；不能仅降低 Info.plist 就承诺旧系统支持。打包前需用户确认此范围。
...
首次执行前确认：1.0.0、Apple 芯片/macOS 26+、接受未公证的安装提示、专用私钥保管目录。目前未创建正式密钥或提供默认测试密钥，缺少公钥将停止构建。
...
```sh
uv run python scripts/release_keys.py generate --directory /Users/leo/.config/sona-release-keys
```
...
```sh
uv run --with pyinstaller==6.22.2 python scripts/build_macos.py --public-key /Users/leo/.config/sona-release-keys/update-public-key.json --confirm-version 1.0.0
```
...
```sh
uv run python scripts/release_keys.py sign --private-key /Users/leo/.config/sona-release-keys/update-private.pem --public-key /Users/leo/.config/sona-release-keys/update-public-key.json --confirm-version 1.0.0 dist/Sona-1.0.0-macos-arm64/Sona-1.0.0-macos-arm64.zip dist/Sona-1.0.0-macos-arm64/Sona-1.0.0-macos-arm64.dmg
```
...
使用本次适配的 ReleaseHub，选择 ZIP（macos / arm64 / zip）和 DMG（macos / arm64 / dmg）。两份 `.sig.json` 留在各安装包旁，不作为安装包选择。

ReleaseHub 在任何远端写入前读取 sidecar、验证签名及元数据，全部附件上传成功后才写 stable.json。签名经 `assets[].updateSignature` 透传，数据库迁移 6 保留该字段。Sona 仓库缺 sidecar 会中止发布；其他产品旧流程不变。ReleaseHub 不保管私钥，sidecar 公钥只用于一致性校验；真正信任锚是 Sona 内置公钥。
...
- 测试代码已补充但未执行：缺失/篡改签名、错误公钥、版本目标不匹配、helper 再验 ZIP、代理 DNS、路径穿越、任务冻结等。
- 用户需完成首次安装、真实 1.0.0 → 1.0.1 升级、数据保留、失败恢复、无开发工具机器上的原生依赖检查。
