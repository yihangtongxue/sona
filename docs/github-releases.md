...
普通分支提交不触发发布。在 **Actions → Build and release Sona → Run workflow** 手动运行时，默认只构建并保存附件，供维护者下载验收；不会读取签名私钥或创建 Release。若标签推送未触发发布，可选择对应的 `v*` 标签并勾选 `publish`，补发同一版本；分支不能执行签名发布。工作流不运行项目测试。未勾选发布的构建附件尚未签名，不能直接作为自动更新源发布。
